...
# Custom-model logic (parse / check / install / download / load) lives in
# `custom_model_shared.py`. The check/install/download actions run in the
# one-shot worker process; the daemon only invokes `load_custom_model`
# when an `action: "load"` arrives with `provider: "custom"`.
load_custom_model = custom_model_shared.load_custom_model
...
def _transcribe_custom(model, audio_path, language):
    ...
    # Third-party transcribe funcs (e.g. mimo_mlx.transcribe) print debug
    # info to stdout (e.g. "Mel: (128, 745)"), which would pollute the JSON
    # protocol. Mirror the other _transcribe_* functions' redirection.
    old_stdout = sys.stdout
    sys.stdout = sys.stderr
    try:
        ...
        if action == "ping":
            ...
            try:
                ...
                if provider == "custom":
                    ...
                    # Third-party load funcs (e.g. mimo_mlx.load_asr) print
                    # to stdout during weight loading, which would pollute
                    # the JSON protocol. Mirror load_model()'s redirection.
                    old_stdout = sys.stdout
                    sys.stdout = sys.stderr
                    try:
