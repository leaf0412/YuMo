...
fn test_get_recording_audio_returns_base64() {
    ...
    let path = voiceink_tauri_lib::recorder::save_recording(&audio, tmp.path()).unwrap();

    let data_uri = voiceink_tauri_lib::recorder::read_recording_as_data_uri(&path).unwrap();
...
fn test_get_recording_audio_not_found() {
    let result = voiceink_tauri_lib::recorder::read_recording_as_data_uri(
