use yumo_lib::{db, text_processor, pipeline, transcriber, platform::recorder, platform::audio_ctrl};
...
fn test_full_app_lifecycle() {
    ...
    // 10. Insert transcription to DB
    let t_id = db::insert_transcription(&conn, "test transcription", 2.5, "ggml-base", 2, None).unwrap();
    let result = db::get_transcriptions(&conn, None, None, 20).unwrap();
    assert_eq!(result.items.len(), 1);

    // 11. Delete transcription
