...
pub fn insert_transcription(
    ...
    // enhanced_text column kept in schema (old DBs); always NULL after AI feature removal.
    conn.execute(
        "INSERT INTO transcriptions (id, text, timestamp, duration, model_name, word_count, recording_path)
         VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7)",
        params![id, text, now, duration, model_name, word_count, recording_path],
    )
    .map_err(|e| {
    ...
    let items: Vec<TranscriptionRecord> = match (cursor, query) {
        (None, None) => {
            let mut stmt = conn.prepare(
                "SELECT id, text, timestamp, duration, model_name, word_count, recording_path
        ...
        (None, Some(q)) => {
            let mut stmt = conn.prepare(
                "SELECT t.id, t.text, t.timestamp, t.duration, t.model_name, t.word_count, t.recording_path
